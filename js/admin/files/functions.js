$(function() {
	var urlrel = $('#uploadfiles_image').attr('rel');
	//===== File manager =====//	
 ...
 $("#uploader_story").pluploadQueue({
	...
	if($("div").is("#uploader_story")){
		...
		if($("a").is("#pickfiles_image")){
		uploader_image = new plupload.Uploader({
			runtimes : 'html5,html4',
			browse_button : 'pickfiles_image',
			container : 'up_image',
			max_file_size : '15mb',
			unique_names : true,
			url : base_url+'admin/upload/index/'+urlrel,
			filters : [
				{title : "Image files", extensions : "jpg,jpeg,gif,png"}
			]
		});
		$('#uploadfiles_image').click(function(e) {
		...
		uploader_image.bind('FilesAdded', function(up, files) {
			  if(up.files.length > 1){
				 alert('Only 1 foto');
				 uploader_image.removeFile(files[0]);
			  }
			  else{
				 $.each(files, function(i, file) {
					$('#filelist_image').append(
						'<div id="' + file.id + '">' +
		...
		uploader_image.bind('Error', function(up, err) {
			$('#filelist_image').append("<div>Error: " + err.code +
				", Message: " + err.message +
		...
		uploader_image.bind('FileUploaded', function(up, file) {
			$('#' + file.id + " b").html("100%");
			$('form.main').append('<input type="hidden" name="uploader_'+$('form.main').data("name")+'_image_tmpname" value="'+file.target_name+'">');
			/*console.log(file);*/
		});
	...
	//===== Dynamic data table =====//
	oTable = $('.dTable_contributros_list,.dTable_contributors_details').dataTable({
		"bJQueryUI": false,
		"bAutoWidth": false,
	...
	oTable = $('#news_data_table').dataTable({
		"bJQueryUI": false,
		"bAutoWidth": false,
		"aLengthMenu": [[10, 20, 25, 50, -1], [10, 20, 25, 50, "All"]],
    "iDisplayLength": 20,
